...
(define-condition key-already-exists (error)
  ())
...
(defclass etcd ()
  ((config :reader config :initarg :config)
   ...
   (debug-trace :reader debug-trace :initform nil)
   (role :reader role)))
...
(defun monitor-etcd-output (etcd s)
  ...
  (with-slots (ready debug-trace version start-semaphore id role on-leader on-follower) etcd
    (when debug-trace
      (format t "~A~%" s))
...
(defmethod initialize-instance :after ((etcd etcd) &key)
  ...
  (with-slots (config debug-trace start-semaphore process get-put-uri) etcd
    (flet ((get-config-value (key)
      ...
      (setf get-put-uri (str:concat (if config
                                        ...
                                        "http://127.0.0.1:2379")
                                    "/v2/keys/"))
      (format t "** NEW VERSION **~%")
      (setf debug-trace (gethash "debug-trace" config))
      (let ((cmd (if config
                     `("etcd"
	...
	(let ((initial-cluster-state (gethash "initial-cluster-state" config)))
	  (when initial-cluster-state
	    (setf cmd (append cmd (list "--initial-cluster-state" initial-cluster-state)))))
...
(defun (setf get-etcd) (value key etcd &key (key-must-not-exist nil))
  "Put the KEY/VALUE pair into ETCD."
  (with-slots (get-put-uri) etcd
    (let ((parameters (list `("value" . ,value))))
      (when key-must-not-exist
        (push '("prevExist" . "false") parameters))
      (multiple-value-bind (answer error-code)
          (drakma:http-request (concatenate 'string get-put-uri key)
                               :method :put
                               :parameters parameters)
        (print (flexi-streams:octets-to-string answer))
        (when (= error-code 412)
          (error 'key-already-exists))
        (when (and (not (= error-code 200)) (not (= error-code 201)))
          (error (format nil "can't store in etcd[~A]: ~A" error-code (flexi-streams:octets-to-string answer))))
        key))))
